...
function logBoxPosition(box: HTMLElement) {
    ...
    try{
        ...
        // Start drag
        sizer.addEventListener('mousedown', function (event: MouseEvent) {
            isDragging = true;
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (!box.contains(event.target as Node) && !sizer.contains(event.target as Node)) {
            sizer.remove();
...
//     console.error('Box element not found');
// }
function logBoxPositionHover(box: HTMLElement){
    if (!box) {
        console.error('Box element not found');
        return;
    }
